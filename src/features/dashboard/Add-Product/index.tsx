...
const AddProduct = () => {
  ...
  const buildPayload = (mode: SubmissionMode): CreateProductPayload => {
  ...
  const handleBasicDetailChange = (next: BasicDetailFormState) => {
    setBasicDetail(next);
  };

  const handleMediaDetailChange = (next: ProductMediaState) => {
    setMediaDetail(next);
  };
